...
async def bang_router(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- 7. СЛУЖЕБНЫЕ КОМАНДЫ ---
        ...
        await format_sword_table(update, "CONQUEROR_ROLLS", CONQUERORS_BLADE_STATS)
        return

    elif command == "doomr":
        await format_sword_table(update, "DOOM_ROLLS", DOOMBRINGER_STATS)
        return

    elif command == "fzr":
        await format_armor_table(update, "FURIOUS_ZEUS_ARMOR", FZH_STATS)
        return

    elif command == "zr":
        await format_armor_table(update, "ZEUS_ARMOR", LZS_STATS)
        return
