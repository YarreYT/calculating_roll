...
ASC_WEAPON_TYPES = {
    'w': {"name": "Wooden Sword", "base_dmg": 10395, "fixed_roll": True, "has_rolls": False},
    'd': {"name": "Dual Daggers", "stats": CONQUERORS_BLADE_STATS, "fixed_roll": False, "min_roll": 6},
    't': {"name": "Poseidon's Trident", "stats": CONQUERORS_BLADE_STATS, "fixed_roll": False, "min_roll": 6},
    'k': {"name": "Lightning Katana", "stats": CONQUERORS_BLADE_STATS, "fixed_roll": False, "min_roll": 6},
    'e': {"name": "Magma's Edge", "stats": CONQUERORS_BLADE_STATS, "fixed_roll": False, "min_roll": 6},
...
async def analyze_weapon_asc(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if is_fixed_roll:
            ...
            roll_text = "У этого типа единственный ролл"
        else:
...
async def w_analyze_weapon_asc(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if is_fixed_roll:
            base_dmg = weapon_info["base_dmg"]
            roll_display = "У этого типа единственный ролл"
        else:
...
async def l_analyze_weapon_asc(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        if is_fixed_roll:
            base_dmg = weapon_info["base_dmg"]
            roll_display = f"У этого типа оружия единственный ролл"
        else:
