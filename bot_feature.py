...
async def analyze_weapon(update, context: ContextTypes.DEFAULT_TYPE, item_key: str):
    ...
    if len(args) in (3, 4):
        ...
        if len(args) == 4:
            ...
            reforge_input_cap = reforge_input.capitalize()
            found_reforge = False
    ...
    if errors:
        example = f"{command_name} {{урон}} {{ур. улучшения}} {{y/n}} [reforge] (если reforge нет - не пишите)"
        error_message = f"🛑 **Обнаружены ошибки формата для {command_name}:**\n"
        error_message += "\n".join(errors)
        error_message += "\n\n**Пример написания:**\n"
        error_message += f"Формат: `{example}` (Макс. ур: {max_lvl})"
...
async def analyze_armor(update, context: ContextTypes.DEFAULT_TYPE, item_key: str):
    ...
    # Check for errors before proceeding to calculation
    if errors:
        example = f"{command_name} {{ХП}} {{ур. улучшения}} {{y/n}}"
        error_message = f"🛑 **Обнаружены ошибки формата для {command_name}:**\n"
        error_message += "\n".join(errors)
        error_message += "\n\n**Пример написания:**\n"
        error_message += f"Формат: `{example}` (Макс. ур: {max_lvl})"
...
async def analyze_full_set(update, context: ContextTypes.DEFAULT_TYPE, item_key: str):
    ...
    # Check for errors before proceeding to calculation
    if errors:
        example = f"{command_name} {{ХП Шлем}} {{ХП Грудь}} {{ХП Ноги}} {{Ур Шлем}} {{Ур Грудь}} {{Ур Ноги}} {{y/n Шлем}} {{y/n Грудь}} {{y/n Ноги}}"
        error_message = f"🛑 **Обнаружены ошибки формата для {command_name}:**\n"
        error_message += "\n".join(errors)
        error_message += "\n\n**Пример написания:**\n"
        error_message += f"Формат: `{example}` (Макс. ур: {max_lvl})"
...
async def w_analyze_weapon(update, context: ContextTypes.DEFAULT_TYPE, item_key: str):
    ...
    # Check for errors before proceeding to calculation
    if errors:
        example = f"{command_name} {{ваш ролл}} > {{желаемый ур. улучшения до {max_lvl}}} {{y/n}} [reforge] (если reforge нет - не пишите)"
        error_message = f"🛑 **Обнаружены ошибки формата для {command_name}:**\n"
        error_message += "\n".join(errors)
        error_message += "\n\n**Пример написания:**\n"
        error_message += f"Формат: `{example}`"
        await update.message.reply_text(error_message, parse_mode=ParseMode.MARKDOWN)
        return
    ...
    try:
        ...
        response = (
            ...
            f"⚔️ Базовый <code>{int(base_dmg_at_roll):,}</code> -> Желаемый <code>{final_dmg:,}</code>\n"
...
async def w_analyze_armor(update, context: ContextTypes.DEFAULT_TYPE, item_key: str):
    ...
    # Check for errors before proceeding to calculation
    if errors:
        example = f"{command_name} {{ваш ролл}} > {{желаемый ур. улучшения до {max_lvl}}} {{y/n}}"
        error_message = f"🛑 **Обнаружены ошибки формата для {command_name}:**\n"
        error_message += "\n".join(errors)
        error_message += "\n\n**Пример написания:**\n"
        error_message += f"Формат: `{example}` (Макс. ур: {max_lvl})"
    ...
    try:
        ...
        response = (
            ...
            f"UPG: <code>{target_level}</code>\n"
            f"Corrupted: <code>{corrupted_text}</code>\n"
            f"❤️ Базовое <code>{int(base_hp_at_roll):,}</code> -> Желаемое <code>{final_hp:,}</code>\n"
            f"💰 Нужно потратить <code>{total_gold_needed:,}</code>"
        )
        await update.message.reply_text(response, parse_mode=ParseMode.HTML)
...
async def w_analyze_full_set(update, context: ContextTypes.DEFAULT_TYPE, item_key: str):
    ...
    # Check for errors before proceeding to calculation
    if errors:
        example = f"{command_name} {{ролл1}} {{ролл2}} {{ролл3}} > {{ур1}} {{ур2}} {{ур3}} {{y/n1}} {{y/n2}} {{y/n3}}"
        error_message = f"🛑 **Обнаружены ошибки формата для {command_name}:**\n"
        error_message += "\n".join(errors)
        error_message += "\n\n**Пример написания:**\n"
        error_message += f"Формат: `{example}` (Макс. ур: {max_lvl})"
    ...
    try:
        ...
        for res in results:
            ...
            response += (
                ...
                f"❤️ Базовое <code>{int(res['base']):,}</code> -> Желаемое <code>{res['final']:,}</code>\n"
                f"\n"
            )
        await update.message.reply_text(response, parse_mode=ParseMode.HTML)
...
*3. Калькулятор желаемых результатов (Прогноз)*
