...
WRONG_TOPIC_TEXTS = [
    ...
    "АЛООО!!! Не сюда!!!! Иди в разрешённый чат",
    "Да что с тобой? Как тебе объяснить что я не здесь работаю! Сука! Иди в разрешённый чат",
    "Да вроде же не глухие и не слепые. Ну, не первый раз же говорю вам, что с командами идите в разрешённый чат",
    "DURA"
]
...
UNKNOWN_COMMAND_RESPONSES = {
    "Такой команды нет. Напиши !crhelp": 20,
    "Чёрный... Ой, то есть такой команды нет. !crhelp": 15,
    "АЛО? Такой команды нет! Пиши !crhelp": 15,
    "Не знаю такой команды. Возможно, ты сам её придумал. Напиши !crhelp и не еби мозги": 10,
    "Я хуею с этой дуры": 1,
}
UNKNOWN_COMMAND_PHOTOS = {
    "Я хуею с этой дуры": "https://www.meme-arsenal.com/memes/0e348db696817986594705050437451b.jpg",
}
...
def find_base_damage_for_asc(dmg: float, level: int, is_corrupted: bool, reforge_mult: float) -> tuple:
    # Вычисляем inferred_base для обычных мечей
    inferred_base = infer_base_for_weapon(dmg, level, is_corrupted, reforge_mult)

    # ПРАВИЛЬНАЯ проверка: насколько 10395 больше inferred_base в процентах
    if inferred_base > 0:
        percent_diff = ((WOODEN_SWORD_OLD_BASE - inferred_base) / inferred_base) * 100
    else:
        percent_diff = float('inf')

    # Если разница <= 4.7619%, то это Wooden Sword
    if percent_diff <= WOODEN_SWORD_THRESHOLD_PERCENT:
        return WOODEN_SWORD_OLD_BASE, 11, True  # возвращаем оригинальную базу 10395
...
async def _handle_asc_callback(query, data_parts, page):
    ...
    dmg = float(data_parts[3])  # Финальный урон, введенный пользователем
    upg = int(data_parts[4])
    corr = data_parts[5] == 'y'
    reforge_n = data_parts[6]
    roll = int(data_parts[7])  # Ролл, определенный при первом расчете
    ...
    if page == "total":
        ...
        text = generate_asc_process_page(active_key, dmg, upg, corr, reforge_n, reforge_mult)
    elif page == "tablet":
    ...
    keyboard = generate_asc_analysis_keyboard(
        ...
        current_page=page, active_weapon=item_key
    )
    parse_mode = ParseMode.MARKDOWN_V2 if page == "tablet" else ParseMode.HTML
    try:
...
def generate_asc_process_page(item_key, final_dmg, upg, corr, reforge_name, reforge_mult, state=""):
    ...
    for r in range(7, 12):
        diff = abs(base_stats[r] - inferred_base)
        if diff < best_diff:
            best_diff = diff
            best_roll = r
    ...
    steps.append("")
    steps.append(f"<b>✓ Выбран ролл:</b> <i>{best_roll}/11</i>\n")
    steps.append(f"<b>✓ BASE DMG:</b> <i>{base_stats[best_roll]:,}</i>")
...
def generate_armor_process_page(item_info: dict,
    ...
    if command in ('fz', 'z') and page_type == "process":
        ...
        # Убираем игровую условность (0.45)
        steps.append("<b>2. Убираем игровую условность (0.45):</b>")
        without_rounding = hp - 0.45
        steps.append(f"<i>  {hp:,.2f} - 0.45 = {without_rounding:,.2f}</i>\n")

        # Убираем Corrupted
        if corrupted:
            steps.append("<b>3. Убираем Corrupted (×1.5):</b>")
            before_corr = without_rounding / 1.5
            steps.append(f"<i>  {without_rounding:,.2f} ÷ 1.50 = {before_corr:,.2f}</i>\n")
        else:
            before_corr = without_rounding
            steps.append("<b>3. Corrupted: Нет (×1.00)</b>\n")

        # Убираем фактор роста
        growth_factor = 1 + 0.047619047619 * upg
        steps.append("<b>4. Расчёт базового HP:</b>")
        steps.append(f"<i>  Фактор роста = 1 + {upg} × 0.047619 = {growth_factor:.10f}</i>")
        inferred_base = before_corr / growth_factor
        steps.append(f"<i>  {before_corr:,.2f} ÷ {growth_factor:.10f} = {inferred_base:,.2f}</i>\n")

        # Находим ролл - ИСПРАВЛЕНО: ищем по inferred_base
        steps.append("<b>5. Определение ролла:</b>")
        steps.append(f"<i>  Инференс: {inferred_base:.2f}</i>")
        steps.append("")

        # Ищем ближайший ролл к inferred_base (не используем find_roll_for_armor)
        best_roll = 1
        min_diff = float('inf')
        for r in range(1, 12):
            val = base_stats[r]
        ...
        steps.append("")
        steps.append(f"<b>✓ Выбран ролл:</b> <i>{roll}/11</i>\n")
        steps.append(f"<b>✓ BASE HP:</b> <i>{base_hp:,}</i>")
...
def generate_armor_part_page(item_info: dict, armor_data: dict, command: str, part: str) -> str:
    ...
    if command in ['fz', 'z']:
        ...
        gold_diff = max(0, calculate_gold(item_info['upgrade_cost_lvl1'], upg2) - calculate_gold(item_info['upgrade_cost_lvl1'], upg1))
        response += f"\n\n<b>📈 Сравнение</b>"
        response += f"\n<b>Прирост HP:</b> <i>+{int(hp_diff):,}</i>"
        response += f"\n<b>Дополнительное золото:</b> <i>{gold_diff:,}</i> 💰"
...
async def generate_armor_results(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if not any(armor_data.values()):
        insults = [
            ...
            "Нету данных - нет конфетки, иди лесом! Если тебе не надо ещё раз писать чёртову команду",
            "Ахахаххаах, ебать. Пиши заново! Без данных тебя даже в дурку не примут",
            "А ты вкурсе что ты везде прожал 3 раза Пропустить? Пиши заново, блять",
            "БЛЯ, НАХУЙ ТЫ ЕБЁШЬ МОЗГИ? ТЫ ВСЁ ПРОСКИПАЛ И РАДИ ЧЕГО? ЗАНОВО!",
            "Я бы желал вам, месье, прописать по чердаку, но мне жаль, что я цифровая моделька. Имейте совесть, не ебите мозг даже мне, и админу. Если вам ненадо вводить, не пишите чёртову команду, сука!",
            "Это что-то типа: ""ХУЕСОСЫ ЕБАНЫЕ! О, кнопка Пропустить"" Свинота.. Пиши заново!"
        ]
