...
async def weapon_analysis_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Универсальный обработчик нажатий кнопок для всех типов анализа оружия
    Поддерживает форматы:
    - asc:item_key:page:dmg:upg:corr:reforge:roll:user_msg_id (9 элементов)
    - wasc:item_key:page:dmg:roll:upg:corr:reforge:user_msg_id (9 элементов)
    - lasc:item_key:page:dmg:roll:curr_upg:curr_corr:curr_ref:des_upg:des_corr:des_ref:user_msg_id (11 элементов)
    - a/w/l:item_key:page:... (старые форматы)
    """
...
# Добавьте эти функции в начало файла (после импортов)
...
def generate_tablet_page(item_info, roll, corr, reforge_mult, reforge_name):
    ...
    block_name = f"{item_info['name'].replace(' ', '_').replace('\'', '').upper()}_TABLET"
    return f"```{block_name}\n{title_line}\n\n{table_content}\n\n{footer}\n```"
...
def get_armor_prompt_text(command: str, stage: str, max_level: int) -> str:
    ...
    base += "<i>Пример написания: (без команды)</i>"
...
def generate_armor_tablet_page(item_info: dict, armor_data: dict, part: str) -> str:
    ...
    block_name = f"{item_info['name'].replace(' ', '_').replace('\'', '').upper()}_{part_key.upper()}_TABLET"
    return f"```{block_name}\n{title_line}\n\n{table_content}\n\n{footer}\n```"
...
async def generate_armor_results(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    keyboard = generate_armor_results_keyboard(command, armor_data, user_msg_id, current_page="total", current_part=first_part)
...
def format_sword_table_text(title, stats_dict, mode="normal"):
    ...
    for level in range(1, 12):
        ...
        if mode == "corrupted":
            val = val * 1.5
        rows.append(f"{level:<5} | {f'{val:g}':<10}")
...
def format_armor_part_table_text(title, stats_dict, part):
    ...
    for level in range(1, 12):
        ...
        corr_val = val * 1.5
        rows.append(f"{level:<5} | {f'{val:g}':<10} | {f'{corr_val:g}':<12}")
...
async def handle_reforge_close_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    Удаляет оба сообщения — бота и пользователя
    """
    query = update.callback_query
    ...
    if command == "roll_id":
        ...
        if len(context.args) < 2:
            await update.message.reply_text(
                ...
                "Пример: `!roll_id 97989 CALCULATE ROLL`",
                parse_mode=ParseMode.MARKDOWN
            )
            return
    ...
    if not is_allowed_thread(update):
        ...
        if cfg and cfg["topics"]:
            ...
            await update.message.reply_text(
                ...
                "Владелец группы может написать `!roll_id {ID} {название}` для настройки.",
                parse_mode=ParseMode.MARKDOWN
            )
        return
...
def main():
    ...
    # === ГРУППА 0: КРИТИЧЕСКИЕ ХЭНДЛЕРЫ (все callback'и и "Да") ===
