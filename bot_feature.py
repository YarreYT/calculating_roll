...
def find_base_damage_for_asc(dmg: float, level: int, is_corrupted: bool, reforge_mult: float) -> tuple:
    """
    НАХОДИТ базовый урон и ролл для ASC оружия через процентное отношение.
    Возвращает (base_dmg, roll, is_wooden_sword)

    Проверка: (10395 - inferred_base) / inferred_base * 100 <= 4.7619%
    """
...
async def _handle_wasc_callback(query, data_parts, page):
    ...
    if page == "total":
        ...
        # ИСПРАВЛЕНИЕ: используем ПРЯМОЙ расчёт для прогноза
...
def generate_forecast_process_page(item_info, roll, upg, corr, reforge_name, reforge_mult):
    """Генерация страницы Process для прогноза оружия (!wconq, !wdoom, !wasc)"""
    base_dmg = item_info['stats'][roll]
    steps = []
    steps.append(f"🧮 <b>Детальные вычисления: {item_info['name']}</b>\n\n")
    ...
    if item_info.get('type') == 'asc_weapon' and item_info.get('weapon_key') == 'ws':
        ...
        steps.append(f"<b>✓ Итоговый урон (без баффа):</b> <i>{final_dmg:,.0f}</i>")
        steps.append(f"<b>✓ Итоговый урон (с баффом):</b> <i>{buffed_dmg:,.0f}</i>")
    else:
...
def generate_compare_process_page(item_info, roll, upg, corr, reforge_mult, reforge_name, state):
    """Генерация Process для сравнения (!lconq, !ldoom, !lasc) - для Actual или Wished"""
    base_dmg = item_info['stats'][roll]
    steps = []
...
def generate_asc_process_page(item_key, final_dmg, upg, corr, reforge_name, reforge_mult, state=""):
    """Генерация Process страницы для ASC оружия (ОБРАТНЫЙ РАСЧЁТ)"""
    item_info = ITEMS_MAPPING[item_key]
    base_stats = item_info['stats']
    ...
    if item_key == "asc_ws":
        ...
        steps.append(f"<b>4. Для Wooden Sword:</b>")
        steps.append(f"<i>  База (без баффа): {base_dmg_without_buff:,.2f}</i>")
        ...
        steps.append("")
        steps.append(f"<b>✓ BASE DMG (с баффом): {base_dmg_with_buff:,.0f}</b>")
        steps.append(f"<b>✓ Табличный DMG: {base_dmg_without_buff:,}</b>")
...
def generate_asc_tablet_page(item_key, roll, corr, reforge_mult, reforge_name):
    ...
    Данные совпадают с Process/Total, округлены до сотых, исправлено смещение
    """
    item_info = ITEMS_MAPPING[item_key]
    max_lvl = item_info['max_level']
    b1 = item_info['upgrade_cost_lvl1']
...
async def armor_results_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ===== ВАЖНОЕ ИЗМЕНЕНИЕ ДЛЯ ОБРАТНОГО РАСЧЁТА =====
    # Генерируем текст
    if page_full == "total":
        text = generate_armor_part_page(item_info, armor_data, command, part)
    elif page_full == "process":
        # Для fz/z передаём ВСЕ данные для обратного расчёта
        # Для wfz/wz/lfz/lz передаём как обычно
