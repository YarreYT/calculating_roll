...
async def unified_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Универсальный обработчик для всех inline кнопок"""
    ...
    prefix = data_parts[0]
    action = data_parts[1]
    user_message_id = int(data_parts[2]) if len(data_parts) > 2 else None

    # Обработка закрытия
    if action == CALLBACK_TABLE_CLOSE or action == "close":
        await query.message.delete()
        # Удаляем сообщение пользователя, если доступно ID
        if user_message_id:
            try:
...
async def reforge_command(update, context):
    # Устанавливаем ширину колонок:
    # Reforge(10), Damage(9), Critical(9), Knock(7)
    # < — влево, > — вправо
    header = f"{'Reforge':<10} | {'Damage':>9} | {'Critical':>9} | {'Knock':>7}"
    separator = "-" * len(header)

    table_lines = [header, separator]

    for r in reforges:
        # Выравниваем статы по правому краю (>), чтобы нули были друг под другом
        # Используем ширину, соответствующую заголовкам
        line = f"{r['name']:<10} | {r['dmg']:>9} | {r['crit']:>9} | {r['knk']:>7}"
        table_lines.append(line)

    table_text = "\n".join(table_lines)

    # Собираем блок. Название REFORGE_LIST будет отображаться в углу блока.
    final_message = (
        f"```СПИСОК_ЗАЧАРОВАНИЙ\n"
        f"{table_text}\n"
        f"```"
    )

    # MarkdownV2 необходим для корректного отображения названия блока
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(unified_callback_handler), group=0)
