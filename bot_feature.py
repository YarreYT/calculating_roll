...
from game_data import (
    ...
    WOODEN_SWORD_BUFF_MULTIPLIER,   # ТОЛЬКО эта константа используется
    WOODEN_SWORD_OLD_BASE          # Для новой логики определения
)
...
def find_base_damage_for_asc(dmg: float, level: int, is_corrupted: bool, reforge_mult: float) -> tuple:
    """
    НАХОДИТ базовый урон и ролл для ASC оружия через отношение 10395 / inferred_base.
    ...
    # Проверяем, является ли это Wooden Sword через отношение
    ratio = WOODEN_SWORD_OLD_BASE / inferred_base

    # Если отношение <= порога (4.7619%), то это Wooden Sword
    if ratio <= WOODEN_SWORD_THRESHOLD_PERCENT:
        return WOODEN_SWORD_OLD_BASE, 11, True  # возвращаем оригинальную базу
...
def generate_asc_total_page(item_key, dmg, upg, corr, reforge_name, reforge_mult, roll, base_dmg):
    ...
    # Для Wooden Sword применяем бафф к базе для отображения и расчетов
    ...
    # ❌ УБИРАЕМ деление на бафф — это и есть тот урон, что видит игрок

    display_base_dmg = calc_base_dmg  # Показываем баффнутую базу
...
def generate_asc_process_page(item_key, roll, upg, corr, reforge_name, reforge_mult, state=""):
    ...
    # Для Wooden Sword показываем бафф
    if item_key == "asc_ws":
        steps.append("<b>1.1 Игровые условности, бафф деревянного меча на 1,111111111111858:</b>")
        buffed_dmg = base_dmg * WOODEN_SWORD_BUFF_MULTIPLIER
        steps.append(f"<i>  {base_dmg:,.2f} × {WOODEN_SWORD_BUFF_MULTIPLIER} = {buffed_dmg:,.2f}</i>\n")
        base_dmg = buffed_dmg  # используем баффнутую базу для дальнейших расчетов
